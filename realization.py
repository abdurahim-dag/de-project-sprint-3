...
def update_mart_d_tables(ti):
    ...
    # d_customer
    cur.execute("""
        truncate production.d_customer cascade;
        ...
        INSERT INTO production.d_customer
        select *
        from all_customer;    
    """)
    conn.commit()
    ...
    # d_calendar
    cur.execute("""
        truncate production.d_calendar cascade;
        with all_dates as (
        ...
        INSERT INTO production.d_calendar
        select
    ...
    # d_item
    cur.execute("""
        truncate production.d_item cascade ;

        INSERT INTO production.d_item
...
def update_mart_f_tables(ti):
    ...
    # f_activity
    cur.execute("""
        truncate production.f_activity cascade ;
        INSERT INTO production.f_activity
        select  ual.action_id
            ,dc.date_id
            ,sum(ual.quantity) as click_number
        from stage.user_activity_log as ual
        left join production.d_calendar as dc on ual.date_time::date = dc.fact_date
    ...
    # f_daily_sales
    cur.execute("""
        truncate production.f_daily_sales cascade;
        INSERT INTO production.f_daily_sales
        select
            dc.date_id,
            uol.item_id,
            uol.customer_id,
        ...
        left join production.d_calendar as dc on uol.date_time::date = dc.fact_date
...
t_create_files_request >> t_check_report >> [t_load_user_order_log, t_load_user_activity_log,
                                             t_load_customer_research] >> t_update_mart_d_tables >> t_update_mart_f_tables
